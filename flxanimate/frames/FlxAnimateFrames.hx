...
class FlxAnimateFrames extends FlxAtlasFrames
{
	...
	public static function parseAtlasSpritemap(Path:String, name:String, frames:FlxAnimateFrames) {
		var curJson:AnimateAtlas = haxe.Json.parse(StringTools.replace(#if sys sys.io.File.getContent('$Path/$name') #else Assets.getText('$Path/$name') #end, String.fromCharCode(0xFEFF), ""));
		#if sys
		var curSpritemap = BitmapData.fromFile('$Path/${curJson.meta.image}');
		#else
